// ==UserScript==
// @name         Sigmally Fixes V2 - #1 Tracker
// @version      2.8.4
...
(() => {
	...
	const settings = (() => {
		const settings = {
			...
			viewportOutline1Color: /** @type {[number, number, number, number]} */ ([1, 0.267, 0.267, 0.35]),
			viewportOutlineSelfColor: /** @type {[number, number, number, number]} */ ([0.2, 0.6, 1, 0.35]),
		...
		*/
		...
		setting('#1 viewport outline', [color('viewportOutline1Color')], () => settings.spectator,
			'Color and opacity of the #1 player\'s viewport outline, drawn on the game canvas and minimap. ' +
			'Set opacity to 0 to disable.');
		setting('Own viewport outline', [color('viewportOutlineSelfColor')], () => true,
			'Color and opacity of your own viewport outline on the minimap. Set opacity to 0 to disable.');
	...
	const ui = (() => {
		...
		ui.tracker = (() => {
			const canvas = document.createElement('canvas');
			canvas.style.cssText = 'position: fixed; bottom: 205px; right: 75px; width: 50px; \
			...
			const ctx = aux.require(
				...
				'Unable to get 2D context for the #1 tracker arrow.',
			);
	...
	const render = (() => {
		...
		const renderGame = () => {
			...
			// check if we are #1 on any tab (with debounce to avoid flicker)
			const _isFirst = lb => lb.length > 0 && lb[0].me;
			if (_isFirst(vision.leaderboard)
				|| _isFirst(world.views.get(world.viewId.secondary)?.leaderboard ?? [])
				|| _isFirst(world.views.get(world.viewId.primary)?.leaderboard ?? [])) {
				lastSeenAsFirst = now;
			}
			const amFirst = now - lastSeenAsFirst < 200;

			(function viewportOutline1() {
				const spectateVision = world.views.get(world.viewId.spectate);
				if (!spectateVision || !settings.spectator) return;
				const specCam = spectateVision.camera;
				if (specCam.tscale <= 0.01) return;
				const [cr, cg, cb, ca] = settings.viewportOutline1Color;
				if (ca <= 0) return;
				if (amFirst) return;

				const vpHalfW = 960 / specCam.tscale;
				const vpHalfH = 540 / specCam.tscale;

				const left = specCam.x - vpHalfW;
				const right = specCam.x + vpHalfW;
				const top = specCam.y - vpHalfH;
				const bottom = specCam.y + vpHalfH;

				gl.useProgram(glconf.programs.tracer);
				gl.bindVertexArray(glconf.tracerVao);

				tracerUboFloats.set([cr, cg, cb, ca, 2]);
				gl.bindBuffer(gl.UNIFORM_BUFFER, glconf.uniforms.Tracer);
				gl.bufferSubData(gl.UNIFORM_BUFFER, 0, tracerUboFloats);
				gl.bindBuffer(gl.UNIFORM_BUFFER, null);
			...
			(function minimap() {
				...
				lastMinimapDraw = now;
				ui.tracker.canvas.style.display = 'none'; // hidden by default; shown only if #1 tracking is active
				...
				// own viewport outline
				if (settings.viewportOutlineSelfColor[3] > 0 && vision.camera.scale > 0.01) {
					const [svr, svg, svb, sva] = settings.viewportOutlineSelfColor;
					const aspect = innerWidth / innerHeight;
					const vpHalfW = aspect * 540 / vision.camera.scale / gameWidth * canvas.width;
					const vpHalfH = 540 / vision.camera.scale / gameHeight * canvas.height;
					const camX = (vision.camera.x - border.l) / gameWidth * canvas.width;
					const camY = (vision.camera.y - border.t) / gameHeight * canvas.height;

					ctx.strokeStyle = aux.rgba2hex6(svr, svg, svb, 1);
					ctx.lineWidth = 1.5 * devicePixelRatio;
					ctx.globalAlpha = sva;
					ctx.strokeRect(camX - vpHalfW, camY - vpHalfH, vpHalfW * 2, vpHalfH * 2);
					ctx.globalAlpha = 1;
				}
				...
				// === #1 Player Tracker (spectate tab) ===
				const spectateVision = world.views.get(world.viewId.spectate);
				if (spectateVision && settings.spectator && spectateVision.camera.tscale > 0 && !amFirst) {
					const specCam = spectateVision.camera;
					const specX = (specCam.x - border.l) / gameWidth * canvas.width;
					const specY = (specCam.y - border.t) / gameHeight * canvas.height;

					// #1 viewport rectangle (use tscale: the server-reported scale, unaffected by local zoom)
					if (specCam.tscale > 0.01 && settings.viewportOutline1Color[3] > 0) {
						const vpHalfW = 960 / specCam.tscale / gameWidth * canvas.width;
						const vpHalfH = 540 / specCam.tscale / gameHeight * canvas.height;

						const [vr, vg, vb, va] = settings.viewportOutline1Color;
						ctx.strokeStyle = aux.rgba2hex6(vr, vg, vb, 1);
						ctx.lineWidth = 1.5 * devicePixelRatio;
						ctx.globalAlpha = va;
						ctx.strokeRect(specX - vpHalfW, specY - vpHalfH, vpHalfW * 2, vpHalfH * 2);

						ctx.fillStyle = aux.rgba2hex6(vr, vg, vb, 1);
						ctx.globalAlpha = va * 0.17;
						ctx.fillRect(specX - vpHalfW, specY - vpHalfH, vpHalfW * 2, vpHalfH * 2);
					}

					// player position in world coords
					const myWorldX = ownN > 0 ? ownX : vision.camera.x;
					const myWorldY = ownN > 0 ? ownY : vision.camera.y;
					const myMapX = (myWorldX - border.l) / gameWidth * canvas.width;
					const myMapY = (myWorldY - border.t) / gameHeight * canvas.height;

					// #1 viewport half-dimensions in world coords
					const vpWorldHalfW = 960 / specCam.tscale;
					const vpWorldHalfH = 540 / specCam.tscale;

					// distance from player to the nearest edge of #1's viewport
					const edgeDx = Math.max(0, Math.abs(specCam.x - myWorldX) - vpWorldHalfW);
					const edgeDy = Math.max(0, Math.abs(specCam.y - myWorldY) - vpWorldHalfH);
					const edgeDist = Math.hypot(edgeDx, edgeDy);
					// normalize relative to viewport size (0 = inside viewport, 1 = one viewport away)
					const vpSize = Math.hypot(vpWorldHalfW, vpWorldHalfH);
					const normDist = vpSize > 0 ? edgeDist / vpSize : 0;

					// direction vector (still toward center, for arrow)
					const dx = specCam.x - myWorldX;
					const dy = specCam.y - myWorldY;

					// distance color: red (inside/close to viewport) → yellow → green (far/safe)
					let cr, cg;
					if (normDist < 0.5) {
						cr = 255; cg = Math.floor(220 * (normDist / 0.5));
					} else {
						const t = Math.min((normDist - 0.5) / 0.8, 1);
						cr = Math.floor(255 * (1 - t)); cg = 255;
					}
					const distColor = `rgb(${cr}, ${cg}, 0)`;

					// dashed arrow line from player to #1
					const lineDist = Math.hypot(specX - myMapX, specY - myMapY);
					if (lineDist > 3) {
						const angle = Math.atan2(specY - myMapY, specX - myMapX);

						ctx.strokeStyle = distColor;
						ctx.lineWidth = 1.5 * devicePixelRatio;
						ctx.globalAlpha = 0.6;
						ctx.setLineDash([3 * devicePixelRatio, 2 * devicePixelRatio]);
						ctx.beginPath();
						ctx.moveTo(myMapX, myMapY);
						ctx.lineTo(specX, specY);
						ctx.stroke();
						ctx.setLineDash([]);

						// arrowhead
						const hl = 4 * devicePixelRatio;
						ctx.fillStyle = distColor;
						ctx.globalAlpha = 0.85;
						ctx.beginPath();
						ctx.moveTo(specX, specY);
						ctx.lineTo(specX - hl * Math.cos(angle - 0.5), specY - hl * Math.sin(angle - 0.5));
						ctx.lineTo(specX - hl * Math.cos(angle + 0.5), specY - hl * Math.sin(angle + 0.5));
						ctx.closePath();
						ctx.fill();
					}

					// #1 position marker (diamond)
					ctx.globalAlpha = 0.9;
					ctx.fillStyle = '#ff4444';
					const ms = 3.5 * devicePixelRatio;
					ctx.beginPath();
					ctx.moveTo(specX, specY - ms);
					ctx.lineTo(specX + ms, specY);
					ctx.lineTo(specX, specY + ms);
					ctx.lineTo(specX - ms, specY);
					ctx.closePath();
					ctx.fill();
					...
					ctx.globalAlpha = 0.85;
					ctx.font
						= `bold ${Math.floor(sectorSize / 4)}px "${sigmod.settings.font || 'Ubuntu'}", Ubuntu`;
					ctx.fillText('#1', specX, specY - ms - 3 * devicePixelRatio);
					ctx.font = `${Math.floor(sectorSize / 3)}px "${sigmod.settings.font || 'Ubuntu'}", Ubuntu`;
					ctx.globalAlpha = 1;

					// === On-screen directional arrow indicator ===
					if (ownN > 0 && lineDist > 3) {
						const tCanvas = ui.tracker.canvas;
						const tCtx = ui.tracker.ctx;
						const tSize = tCanvas.width = tCanvas.height = Math.ceil(50 * devicePixelRatio);
						const tCenter = tSize / 2;
						...
						tCtx.fillStyle = '#0004';
						tCtx.beginPath();
						tCtx.arc(tCenter, tCenter, tCenter - 2, 0, Math.PI * 2);
						tCtx.fill();

						// colored ring indicating distance
						tCtx.strokeStyle = distColor;
						tCtx.lineWidth = 3 * devicePixelRatio;
						tCtx.globalAlpha = 0.8;
						tCtx.beginPath();
						tCtx.arc(tCenter, tCenter, tCenter - 2, 0, Math.PI * 2);
						tCtx.stroke();

						// arrow chevron pointing toward #1
						const arrowAngle = Math.atan2(dy, dx);
						const arrowLen = tCenter * 0.6;
						tCtx.save();
						tCtx.translate(tCenter, tCenter);
						tCtx.rotate(arrowAngle);
						tCtx.fillStyle = distColor;
						tCtx.globalAlpha = 0.9;
						tCtx.beginPath();
						tCtx.moveTo(arrowLen, 0);
						tCtx.lineTo(-arrowLen * 0.4, -arrowLen * 0.5);
						tCtx.lineTo(-arrowLen * 0.15, 0);
						tCtx.lineTo(-arrowLen * 0.4, arrowLen * 0.5);
						tCtx.closePath();
						tCtx.fill();
						tCtx.restore();
