...
(() => {
	...
	const settings = (() => {
		const settings = {
			...
			// Scout vision settings
			scoutEnabled: false,
			scoutRelayUrl: '', // e.g. 'wss://my-relay.example.com/relay'
			scoutDistance: 3000,
			/** @type {string[]} */
			scoutDirections: ['N', 'S', 'E', 'W'],
		...
		*/
		...
		separator('• scout vision •');
		setting('Enable scout connections', [checkbox('scoutEnabled')], () => true,
			'Connect extra tabs in spectate mode positioned around you (N, S, E, W, diagonals) to ' +
			'see more of the map. Requires a relay server URL to bypass the per-IP connection limit.');
		{
			// relay URL text input
			const relayListen = input => {
				onSyncs.push(() => input.value = settings.scoutRelayUrl);
				input.value = settings.scoutRelayUrl;
				input.addEventListener('input', () => {
					settings.scoutRelayUrl = input.value;
					settingsExt.save();
				});
			};
			const relayVanilla = fromHTML(
				'<input id="sf-scoutRelayUrl" placeholder="wss://relay.example.com/relay" type="text" />');
			const relaySigmod = fromHTML(
				'<input class="modInput" id="sfsm-scoutRelayUrl" placeholder="wss://relay.example.com/relay" ' +
				'style="border: 1px solid transparent; width: 250px;" type="text" />');
			relayListen(/** @type {HTMLInputElement} */ (relayVanilla));
			relayListen(/** @type {HTMLInputElement} */ (relaySigmod));
			setting('Relay server URL', [{ sigmod: relaySigmod, vanilla: relayVanilla }],
				() => settings.scoutEnabled,
				'WebSocket relay URL. The relay forwards traffic to the game server from a different IP. ' +
				'Format: wss://host/path — target game URL is appended as ?target=...');
		}
		setting('Scout distance', [slider('scoutDistance', 3000, 500, 10000, 100, 0)],
			() => settings.scoutEnabled,
			'How far (in game units) each scout is positioned from your center. Larger values give more ' +
			'coverage but may show less detail around you.');
		{
			// scout directions multi-select checkboxes
			const dirs = ['N', 'S', 'E', 'W', 'NE', 'NW', 'SE', 'SW'];
			const makeDirCheckboxes = (isSigmod) => {
				const container = document.createElement('div');
				container.style.cssText = 'display: flex; flex-wrap: wrap; gap: 4px;';
				for (const dir of dirs) {
					const label = document.createElement('label');
					label.style.cssText = 'font-size: 11px; cursor: pointer; padding: 1px 4px; ' +
						'border: 1px solid #888; border-radius: 3px; user-select: none;' +
						(isSigmod ? ' color: #fff;' : '');
					const cb = document.createElement('input');
					cb.type = 'checkbox';
					cb.style.cssText = 'display: none;';
					cb.checked = settings.scoutDirections.includes(dir);
					const update = () => {
						label.style.background = cb.checked
							? (isSigmod ? '#6871f1' : '#4488ff') : 'transparent';
					};
					update();
					onSyncs.push(() => {
						cb.checked = settings.scoutDirections.includes(dir);
					...
					cb.addEventListener('change', () => {
						if (cb.checked) {
							if (!settings.scoutDirections.includes(dir)) settings.scoutDirections.push(dir);
						} else {
							settings.scoutDirections = settings.scoutDirections.filter(d => d !== dir);
						}
						update();
						settingsExt.save();
					});
			...
			const vanDirs = makeDirCheckboxes(false);
			const smDirs = makeDirCheckboxes(true);
			setting('Scout directions', [{ vanilla: vanDirs, sigmod: smDirs }],
				() => settings.scoutEnabled,
				'Which directions to place scout tabs. Toggle each direction on/off.');
		}
	...
	const ui = (() => {
		...
		ui.perftab = (() => {
			...
			const pollTab = (view, enabled) => {
				...
				if (!oldTab && enabled) {
					...
					if (view === world.viewId.primary) title = '#1';
						...
						// check if it's a scout tab
						for (const [dir, tab] of Object.entries(SCOUT_DIRECTION_MAP)) {
							if (tab === view) { title = dir; break; }
						}
					}
			...
			setInterval(() => {
				...
				// poll scout tabs in perf overlay
				for (const dir of settings.scoutDirections) {
					const tab = SCOUT_DIRECTION_MAP[dir];
					if (tab) pollTab(tab, settings.scoutEnabled);
				}
			}, 500);
	...
	const TAB_PRIMARY = Symbol('primary');
	const TAB_SECONDARY = Symbol('secondary');
	const TAB_SPECTATE = Symbol('spectate');

	// Scout tab symbols for extra vision connections
	const TAB_SCOUT_N = Symbol('scout-N');
	const TAB_SCOUT_S = Symbol('scout-S');
	const TAB_SCOUT_E = Symbol('scout-E');
	const TAB_SCOUT_W = Symbol('scout-W');
	const TAB_SCOUT_NE = Symbol('scout-NE');
	const TAB_SCOUT_NW = Symbol('scout-NW');
	const TAB_SCOUT_SE = Symbol('scout-SE');
	const TAB_SCOUT_SW = Symbol('scout-SW');
	const ALL_SCOUT_TABS = [
		TAB_SCOUT_N, TAB_SCOUT_S, TAB_SCOUT_E, TAB_SCOUT_W,
		TAB_SCOUT_NE, TAB_SCOUT_NW, TAB_SCOUT_SE, TAB_SCOUT_SW,
	];
	const SCOUT_DIRECTION_MAP = {
		'N': TAB_SCOUT_N, 'S': TAB_SCOUT_S, 'E': TAB_SCOUT_E, 'W': TAB_SCOUT_W,
		'NE': TAB_SCOUT_NE, 'NW': TAB_SCOUT_NW, 'SE': TAB_SCOUT_SE, 'SW': TAB_SCOUT_SW,
	};
	/** @type {Map<symbol, [number, number]>} - unit direction vectors */
	const SCOUT_OFFSETS = new Map([
		[TAB_SCOUT_N,  [0, -1]],
		[TAB_SCOUT_S,  [0, 1]],
		[TAB_SCOUT_E,  [1, 0]],
		[TAB_SCOUT_W,  [-1, 0]],
		[TAB_SCOUT_NE, [0.707, -0.707]],
		[TAB_SCOUT_NW, [-0.707, -0.707]],
		[TAB_SCOUT_SE, [0.707, 0.707]],
		[TAB_SCOUT_SW, [-0.707, 0.707]],
	]);
	// all view tabs that can own cells — used for dynamic dead-checks
	const ALL_VIEW_TABS = [TAB_PRIMARY, TAB_SECONDARY, TAB_SPECTATE, ...ALL_SCOUT_TABS];
	/**
	 * @typedef {{
	 * 	id: number,
	...
	const world = (() => {
		...
		world.viewId = {
			primary: TAB_PRIMARY, secondary: TAB_SECONDARY, spectate: TAB_SPECTATE,
			scoutN: TAB_SCOUT_N, scoutS: TAB_SCOUT_S, scoutE: TAB_SCOUT_E, scoutW: TAB_SCOUT_W,
			scoutNE: TAB_SCOUT_NE, scoutNW: TAB_SCOUT_NW, scoutSE: TAB_SCOUT_SE, scoutSW: TAB_SCOUT_SW,
		};
		...
		world.killCell = (view, cell, killerId, now) => {
			...
			if (killerId) {
				...
				// check all view tabs dynamically (supports scout tabs)
				let allCellDead = true;
				for (const tab of ALL_VIEW_TABS) {
					if (cell[tab] && cell[tab][CELL_BORN] && !cell[tab][CELL_DEADAT]) {
						allCellDead = false; break;
					}
				}
				if (allCellDead) {
					cell.deadAt = now;
		...
		world.killPellet = (view, pellet, killerId, now) => {
			...
			if (killerId) {
				...
				// only mark dead if all views see it dead
				let allPelletDead = true;
				for (const tab of ALL_VIEW_TABS) {
					if (pellet[tab] && pellet[tab][PELLET_BORN] && !pellet[tab][PELLET_DEADAT]) {
						allPelletDead = false; break;
					}
				}
				if (allPelletDead) {
					pellet.deadAt = now;
	...
	const net = (() => {
		...
		*/
						...
						while (true) {
							...
							if (isPellet) {
								...
								if (pellet) {
									...
									// add scout tab records dynamically
									for (const st of ALL_SCOUT_TABS) {
										pellet[st] = new Float32Array(PELLET_RECORD_SIZE);
									}
								...
								if (cell) {
									...
									// add scout tab records dynamically
									for (const st of ALL_SCOUT_TABS) {
										cell[st] = new Float32Array(CELL_RECORD_SIZE);
									}
		...
		/**
		 * Connect to the game server through a relay proxy.
		 * The relay URL should accept a query param ?target= with the real game server URL.
		 * @param {symbol} view
		 * @returns {WebSocket | undefined}
		 */
		const connectViaRelay = (view) => {
			if (net.connections.get(view)?.ws) return;
			if (!settings.scoutRelayUrl) {
				console.error('[Scout] No relay URL configured');
				return;
			}

			const targetUrl = net.url();
			const separator = settings.scoutRelayUrl.includes('?') ? '&' : '?';
			const relayWsUrl = settings.scoutRelayUrl + separator + 'target=' + encodeURIComponent(targetUrl);
			const fakeUrl = /** @type {any} */ ({ includes: () => false, toString: () => relayWsUrl });
			let ws;
			try {
				ws = new WebSocket(fakeUrl);
			} catch (err) {
				console.error('[Scout] Can\'t connect to relay:', err);
				return;
			}
			if (net.connections.get(view)) net.connections.get(view).ws = ws;
			...
			ws.binaryType = 'arraybuffer';
			ws.addEventListener('close', e => {
				console.error('[Scout] WebSocket closed:', e);
				const connection = net.connections.get(view);
				const vision = world.views.get(view);
				if (!connection || !vision) return;
				...
				// reconnect after delay if scouts are still enabled
				if (settings.scoutEnabled && SCOUT_OFFSETS.has(view)) {
					const delay = connection.rejections >= 5 ? 5000 : 1500;
					setTimeout(() => {
						if (settings.scoutEnabled) connectViaRelay(view);
					}, delay);
				}
			});
			ws.addEventListener('error', () => {});
			// reuse the existing message handler by dispatching through the same logic
			ws.addEventListener('message', e => {
				const connection = net.connections.get(view);
				const vision = world.views.get(view);
				if (!connection || !vision) return ws.close();
				...
				if (!connection.handshake) {
					...
					connection.handshake = { shuffle, unshuffle };
					// immediately enter spectate mode
					net.play(view, { name: '', skin: '', clan: aux.userData?.clan, state: 2 });
					return;
				}
				...
				let o = 1;
				switch (dat.getUint8(0)) {
					case 0x10: { // world update — same parsing as main handler
						const killCount = dat.getUint16(o, true);
						o += 2;
						for (let i = 0; i < killCount; ++i) {
							const killerId = dat.getUint32(o, true);
							...
							if (isPellet) {
								...
								const xyr = world.xyr(killed, undefined, now);
								killed.vx = xyr.x; killed.vy = xyr.y; killed.vr = xyr.r; killed.vjr = xyr.jr;
								killed.vweight = xyr.weight; killed.vupdated = now;
								world.killCell(view, killed, killerId, now);
							}
						}
						...
						while (true) {
							...
							const red = (flags & 0x02) ? dat.getUint8(o++) : 0;
							const green = (flags & 0x02) ? dat.getUint8(o++) : 0;
							const blue = (flags & 0x02) ? dat.getUint8(o++) : 0;
							let skin; if (flags & 0x04) [skin, o] = aux.readZTString(dat, o);
							let name; if (flags & 0x08) [name, o] = aux.readZTString(dat, o);
							if (flags & 0x08) [name] = aux.parseName(name ?? '');
							let clan; [clan, o] = aux.readZTString(dat, o);
							const jagged = !!(flags & 0x01);
							...
							if (r <= 20 && !jagged && !sub) {
								...
								if (cell) {
									if (flags & 0x02) {
										cell.red = red / 255; cell.green = green / 255; cell.blue = blue / 255;
									}
									if (flags & 0x08) cell.name = name || 'An unnamed cell';
									if (flags & 0x04) cell.skin = skin ?? '';
									cell.clan = clan; cell.jagged = jagged; cell.sub = sub;
									...
									if (record) {
										...
										record[CELL_DEADAT] = record[CELL_DEADTO] = 0;
										if (cell.deadAt) {
											cell.born = now; cell.deadAt = cell.deadTo = 0;
											cell.vx = x; cell.vy = y; cell.vr = cell.vjr = r; cell.vweight = 0;
										}
										record[CELL_TOPINDEX] = (record[CELL_TOPINDEX] + 1) & CELL_TOPINDEX_MASK;
										record[3 * record[CELL_TOPINDEX]] = x;
									...
									const { x: ix, y: iy, r: ir, jr, weight } = world.xyr(cell, undefined, now);
									cell.vx = ix; cell.vy = iy; cell.vr = ir; cell.vjr = jr;
									cell.vweight = weight; cell.vupdated = now;
									cell.tx = x; cell.ty = y; cell.tr = r;
								} else {
									const hash = String(red << 16 | green << 8 | blue) + name;
									world.cells.set(id, cell = {
										id,
									...
									const record = cell[view];
									record[CELL_BORN] = now;
									record[0] = x; record[1] = y; record[2] = r;
								}
							}
						}
						...
						while (true) {
							...
							o += 4;
							if (id === 0) break;
							const isPellet2 = !!world.pellets.get(id);
							const dead = world.pellets.get(id) ?? world.cells.get(id);
							if (!dead) continue;
							if (isPellet2) world.killPellet(view, dead, 0, now);
							else world.killCell(view, dead, 0, now);
						}
						break;
					}

					case 0x11: { // camera update from spectate
						vision.camera.tx = dat.getFloat32(o, true);
						vision.camera.ty = dat.getFloat32(o + 4, true);
						vision.camera.tscale = dat.getFloat32(o + 8, true);
						break;
		...
		/** Helper to create a scout connection (view + net entry using relay) */
		net.createScout = view => {
			if (net.connections.has(view)) return;
			world.create(view);
			// create connection object first so it exists when event handlers fire
			net.connections.set(view, {
				handshake: undefined,
				latency: undefined,
				pinged: undefined,
			...
			connectViaRelay(view); // assigns ws to the existing connection entry
		};

		/** Remove a scout connection cleanly */
		net.destroyScout = view => {
			const con = net.connections.get(view);
			if (con?.ws && con.ws.readyState !== WebSocket.CLOSED && con.ws.readyState !== WebSocket.CLOSING) {
				con.ws.close();
			}
			net.connections.delete(view);
			world.views.delete(view);
			input.views.delete(view);
			const now = performance.now();
			for (const cell of world.cells.values()) world.killCell(view, cell, 0, now);
			for (const pellet of world.pellets.values()) world.killPellet(view, pellet, 0, now);
			render.uploadPellets = true;
		...
		setInterval(() => {
			for (const [view, connection] of net.connections) {
				if (!connection.ws) continue;
				if (connection.ws.readyState !== WebSocket.CONNECTING && connection.ws.readyState !== WebSocket.OPEN)
					continue;
				// scout connections go through relay, their URL will differ — skip them
				if (SCOUT_OFFSETS.has(view)) continue;
		...
		// Scout vision management
		/** @type {Set<symbol>} active scout view IDs */
		const activeScouts = new Set();
		setInterval(() => {
			const now = performance.now();
			if (settings.scoutEnabled && settings.scoutRelayUrl) {
				// determine which scout tabs should be active based on settings.scoutDirections
				const desiredScouts = new Set();
				for (const dir of settings.scoutDirections) {
					const tab = SCOUT_DIRECTION_MAP[dir];
					if (tab) desiredScouts.add(tab);
				}

				// create new scouts
				for (const tab of desiredScouts) {
					if (!activeScouts.has(tab)) {
						activeScouts.add(tab);
						net.createScout(tab);
					}
				}

				// destroy removed scouts
				for (const tab of activeScouts) {
					if (!desiredScouts.has(tab)) {
						activeScouts.delete(tab);
						net.destroyScout(tab);
					}
				}

				// position each scout's spectate camera toward the offset from the player
				const primaryVision = world.views.get(world.viewId.primary);
				const playerX = primaryVision?.camera?.tx ?? 0;
				const playerY = primaryVision?.camera?.ty ?? 0;

				for (const tab of activeScouts) {
					const con = net.connections.get(tab);
					if (!con?.handshake || con.ws?.readyState !== WebSocket.OPEN) continue;
					const offset = SCOUT_OFFSETS.get(tab);
					if (!offset) continue;

					const targetX = playerX + offset[0] * settings.scoutDistance;
					const targetY = playerY + offset[1] * settings.scoutDistance;

					// send move packet to steer spectate camera toward target
					net.move(tab, Math.round(targetX), Math.round(targetY));
				}
			} else {
				// destroy all scouts if disabled
				for (const tab of activeScouts) {
					net.destroyScout(tab);
				}
				activeScouts.clear();
	...
	const render = (() => {
		...
		const renderGame = () => {
			...
			(function minimap() {
				...
				// draw scout viewport indicators on minimap
				if (settings.scoutEnabled && border) {
					const scoutColors = {
						'N': '#44aaff', 'S': '#44ff44', 'E': '#ffaa44', 'W': '#ff44ff',
						'NE': '#44ffff', 'NW': '#ffff44', 'SE': '#aa44ff', 'SW': '#ff8888',
					};
					for (const dir of settings.scoutDirections) {
						const tab = SCOUT_DIRECTION_MAP[dir];
						if (!tab) continue;
						const scoutVision = world.views.get(tab);
						if (!scoutVision || !scoutVision.camera.tscale) continue;

						const sx = (scoutVision.camera.x - border.l) / gameWidth * canvas.width;
						const sy = (scoutVision.camera.y - border.t) / gameHeight * canvas.height;

						const viewW = (1920 / scoutVision.camera.tscale) / gameWidth * canvas.width;
						const viewH = (1080 / scoutVision.camera.tscale) / gameHeight * canvas.height;

						ctx.strokeStyle = scoutColors[dir] || '#888888';
						...
						// label
						ctx.globalAlpha = 0.7;
						ctx.fillStyle = scoutColors[dir] || '#888888';
						ctx.font = `${9 * devicePixelRatio}px Ubuntu`;
						ctx.textAlign = 'center';
						ctx.fillText(dir, sx, sy + 3 * devicePixelRatio);
						ctx.globalAlpha = 1;
					}
				}
			})();
	...
	window.sigfix = { aux, sigmod, ui, settings, world, net, input, glconf, render,
		SCOUT_DIRECTION_MAP, SCOUT_OFFSETS, ALL_SCOUT_TABS };
})();
